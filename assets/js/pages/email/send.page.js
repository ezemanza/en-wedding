    ...
    handleParsingForm: function() {
      ...
      if(!this.formData.recipients) {
        ...
        if (this.formData.recipients === 'all') {
          argins.recipients = this.guests.map(guest => guest.id);
        } else if (this.formData.recipients === 'main') {
          argins.recipients = this.guests.filter(guest => guest.type === 'main').map(guest => guest.id);
        } else if (this.formData.recipients === 'single') {
          if (!this.formData.single) {
            this.formErrors.single = true;
          } else {
            argins.recipients = [this.formData.single];
          }
        }
      }
